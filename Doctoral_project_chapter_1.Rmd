---
title: "Effect of plasticity on adaptive evolution - mecanism"
...
datapath <- setwd("C:/Users/Emerson Júnior/Google Drive/Doutorado/R/Doctotal Project/WorldCenter")
...
config$gen3sis$ecology$apply_ecology <- function(abundance, traits, landscape, config) {
  ...
  plasticity2 <- function(x, land) {
    ...
    traits_sub <- lapply(traits[, 'temp'], plasticity, plast = 0.02)
...
finalresult <- data.frame(replications = runif(rep, 0, 0),
                          speciation = runif(rep, 0, 0),
                          extinction = runif(rep, 0, 0),
                          diversif = runif(rep, 0, 0),
                          traitevolution = runif(rep, 0, 0))
...
# 
#   if (state == "lowtemp") {
#   val$data$landscape$environment[, 1] <-  val$data$landscape$environment[, 1] + 0.3
#   val$data$inputs$environments$temp  <- val$data$inputs$environments$temp + 0.3} else {
#     print("there is not any state low")
#   }
# 
# if (state == "mediumtemp") {
#   val$data$landscape$environment[, 1] <-  val$data$landscape$environment[, 1] + 0.6
#   val$data$inputs$environments$temp  <- val$data$inputs$environments$temp + 0.6} else {
#     print("there is not state medium")
#   }
...
rm(val, sgen3sis, rateextinction, ratespeciation, diversification)
#rm(list=ls())
...
##### PATHWAY ####
setwd("C:/Users/Emerson Júnior/Google Drive/Doutorado/R/Doctotal Project/WorldCenter/output/config_worldcenter/traits")

caminho <- "C:/Users/Emerson Júnior/Google Drive/Doutorado/R/Doctotal Project/WorldCenter/output/config_worldcenter/traits"

listfiles <- list.files("C:/Users/Emerson Júnior/Google Drive/Doutorado/R/Doctotal Project/WorldCenter/output/config_worldcenter/traits")

filestoread <- length(list.files("C:/Users/Emerson Júnior/Google Drive/Doutorado/R/Doctotal Project/WorldCenter/output/config_worldcenter/traits"))
...
### lapply to select only temp column in group of data ###
coluna <- function(dados, colu){
  result <- lapply(dados, colu)
  return(result)
}
...
for(i in 1:length(datafinal)){
  datafinal[[i]] <- coluna(datafinal[[i]], colunatemp)
}
...
#### loop utilizando lista dentro de lista, como fazer? subscripts out of bounds ## perguntar a bruno || outra coisa, problema no loop com mudança de temperatura
